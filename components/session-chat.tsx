...
export function SessionChat({ canInteract }: { canInteract: boolean }) {
  // Added canInteract prop
      ...
      try {
        ...
        if (data) {
          const formattedMessages = data.map((msg: any) => {
            const userEmail = msg.user?.email || msg.users?.email || msg.auth?.users?.email || "Usuário"
      ...
      <CardFooter className="p-3 border-t">
        <form onSubmit={sendMessage} className="flex w-full gap-2">
          <Input
            ...
            disabled={isLoading || !user || !canInteract} // Disabled based on canInteract
            className="flex-1"
          />
          <Button type="submit" disabled={isLoading || !user || !newMessage.trim() || !canInteract}>
            {" "}
            {/* Disabled based on canInteract */}
            Enviar
          </Button>
        </form>
      </CardFooter>
