...
export function TenderTeamManagement({
  tender,
  agencyUsers,
  currentProfileId,
}: TenderTeamManagementProps) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const { success, message } = await updateTenderTeam(
      tender.id,
      selectedPregoeiro,
      selectedTeamMembers
    );
  ...
  const pregoeiroOptions = agencyUsers.map((user) => ({
    value: user.id,
    label: user.name || user.email || "Usuário Desconhecido",
  }));
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Defina o pregoeiro e os membros da equipe de apoio para esta licitação.
        </CardDescription>
      </CardHeader>
