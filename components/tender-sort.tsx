...
export function TenderSort({ onSortChange, currentSort }: TenderSortProps) {
  const sortOptions = [
    ...
    { value: "title", label: "Título", icon: currentSort.direction === "asc" ? ArrowUpAZ : ArrowDownAZ },
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button variant="outline" size="icon" onClick={toggleDirection}>
        {currentSort.direction === "asc" ? <ArrowUpAZ className="h-4 w-4" /> : <ArrowDownAZ className="h-4 w-4" />}
      </Button>
    </div>
  )
}
