...
export function MeEppTiebreakerHistory({
  tenderId,
  lotId,
  supplierId,
}: MeEppTiebreakerHistoryProps) {
  ...
  useEffect(() => {
    const fetchTiebreakers = async () => {
      ...
      try {
        let query = supabase
          .from("me_epp_tiebreakers")
          .select("*")
          .order("tiebreaker_date", { ascending: false });
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          <p className="text-center text-muted-foreground">
            Nenhum histórico de desempate ME/EPP encontrado.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Registro de desempates fictos conforme Lei Complementar 123/2006
        </CardDescription>
      </CardHeader>
      <CardContent>
