...
export async function getAgencyUsers(agencyId: string) {
  ...
  if (
    profileError ||
    !profile ||
    (profile.profile_type !== "agency" && profile.profile_type !== "admin")
  ) {
...
export async function updateTenderTeam(
  tenderId: string,
  pregoeiroId: string | null,
  teamMembers: string[]
) {
  const supabase = createServerClient();
  ...
  if (
    profileError ||
    !currentProfile ||
    (currentProfile.role !== "agency" && currentProfile.role !== "admin")
  ) {
  ...
  if (currentProfile.role === "agency" && currentProfile.agency_id !== tender.agency_id) {
    return {
      success: false,
      message: "Unauthorized: You can only manage teams for your agency's tenders.",
    };
  }
  ...
  if (allTeamIds.length > 0) {
    ...
    if (invalidUsers.length > 0) {
      return {
        success: false,
        message: "Selected pregoeiro or team members do not belong to this agency.",
      };
    }
  }
