...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!profile || profile.role !== "supplier") {
      return NextResponse.json({ error: "Apenas fornecedores podem participar de licitações" }, { status: 403 })
    }
    ...
    if (existingParticipation) {
      return NextResponse.json({ error: "Fornecedor já está registrado como participante" }, { status: 400 })
    }
    ...
    const { data: tender } = await supabase.from("tenders").select("title, number").eq("id", params.id).single()

    // Enviar notificação para a equipe da licitação
    if (tender) {
      const { data: teamMembers } = await supabase.from("tender_team").select("user_id").eq("tender_id", params.id)
