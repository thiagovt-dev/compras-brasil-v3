...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!content || content.trim() === "") {
      return NextResponse.json({ error: "O conteúdo do esclarecimento é obrigatório" }, { status: 400 })
    }
    ...
    if (now > deadline) {
      return NextResponse.json({ error: "O prazo para esclarecimentos já expirou" }, { status: 400 })
    }
    ...
    const { data: tenderTeam } = await supabase.from("tender_team").select("user_id").eq("tender_id", params.id)
