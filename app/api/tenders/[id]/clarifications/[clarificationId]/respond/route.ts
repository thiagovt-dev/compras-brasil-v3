...
export async function POST(request: NextRequest, { params }: { params: { id: string; clarificationId: string } }) {
  try {
    ...
    const { data: tender } = await supabase.from("tenders").select("title").eq("id", params.id).single()

    // Notificar o usuário que fez o esclarecimento
    await createNotification({
      userId: clarification.user_id,
      title: "Resposta ao esclarecimento",
      message: `Seu pedido de esclarecimento para a licitação ${tender?.title || "N/A"} foi respondido`,
      type: "tender",
      entityId: params.id,
      entityType: "tender",
    })
